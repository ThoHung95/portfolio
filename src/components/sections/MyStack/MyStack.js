...
const stacks = [
  {
    ...
    title: "backend",
    icon: <FiServer />,
    items: [<PostgreSqlButton />, <MongoDbButton />, <ExpressButton />],
  },
  {
    id: "frontend",
    title: "frontend",
    icon: <FiMonitor />,
    items: [<ReactButton />, <BootstrapButton />],
  },
  {
    id: "project",
    title: "project",
    icon: <FiDatabase />,
    items: [<SmartBrainButton />, <BgGeneratorButton />, <RobofriendsButton />],
  },
];
...
function MyStack() {
  ...
  const [openStack, changeOpenStack] = useState({
    open: false,
    stack: stacks[0],
  });
    ...
    <Container maxW={"7xl"} textAlign="center">
      ...
      >
        <Stack flex={1} spacing={{ base: 5, md: 10 }}>
          ...
          {/* PRESENTATION */}
          <Text color={"gray.300"}>{t("my_stack_presentation")}</Text>
          ...
          <Box
            ...
            opacity={0.8}
          >
            <HStack flex={1} spacing={{ base: 5, md: 10 }} height="full">
              ...
              >
                {stacks.map((stack, index) => {
                  return (
                    <Tooltip
                      key={index}
                      label={t("click_stack_btn").replace(
                        "{key}",
                        t(stack.title)
                      )}
                      closeOnClick={true}
